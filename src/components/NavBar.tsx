    ...
    return (
        <>
            <nav className='flex justify-center gap-14 my-3 mx-10 text-greenCustom font-mono'>
                <NavLinkWithBorder exact to="/">Home</NavLinkWithBorder>
...
const NavLinkWithBorder: React.FC<NavLinkProps> = ({ to, exact, children }) => {
    ...
    return (
        <NavLink
            to={to}
            className={`relative pb-1 ${isActive ? 'border-b-2 border-greenCustom hover:border-aquamarine' : ''}`}
        >
            <h2 className='font-bold hover:scale-105 hover:text-aquamarine duration-300'>{children}</h2>
        </NavLink>
    );
};
